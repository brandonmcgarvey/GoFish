...
namespace GoFish
{
    ...
    {        
        ...
        /// <summary>
        /// Gets up to five cards from the stock and sort hand.
        /// </summary>
        ...
        {
            var match = hand.Where(card => card.Value == value)
                .OrderBy(card => card.Suit);
            hand = hand.Where(card => card.Value != value).ToList();   // This removes cards that were matched by reassigning hand to hand (ToList) containing cards that did not match value
        ...
        public void AddCardsAndPullOutBooks(IEnumerable<Card> cards)
        {
            ...
            books.AddRange(groupedBooks);
            books.Sort();

            hand = hand.Where(card => !books.Contains(card.Value)).ToList();
        }
        /// <summary>
        /// Draws a card from the stock and add it to the player's hand and sorts hand
        /// </summary>
